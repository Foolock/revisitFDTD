    ...
    int Nx, int Ny, float dx, bool value)
{
    ...
    int i_min = std::max(0, (int) floor((x_min + Lx / 2) / dx));
    int i_max = std::min(Nx - 1, (int) ceil((x_max + Lx / 2) / dx));

    int j_min = std::max(0, (int) floor((y_min + Ly / 2) / dx));
    int j_max = std::min(Ny - 1, (int) ceil((y_max + Ly / 2) / dx));
...
// Geometric utility function: add a PhC cavity
void create_PhC_cavity_in_mask(bool* mask, float Lx, float Ly, int Nx, int Ny, float dx, 
    float xc, float yc, int N_cavity, float a_lattice, float r_hole)
{
    // Calculate the cavity length
    float L_cavity = N_cavity * a_lattice;  // Edge length of square slab
    // Write the square slab into mask
    float cavity_xmin = xc - L_cavity / 2.0;
    float cavity_xmax = cavity_xmin + L_cavity;
    float cavity_ymin = yc - L_cavity / 2.0;
    float cavity_ymax = cavity_ymin + L_cavity;

    std::vector<Point2D> slab_vertices = {
        {cavity_xmin, cavity_ymin},
        {cavity_xmax, cavity_ymin},
        {cavity_xmax, cavity_ymax},
        {cavity_xmin, cavity_ymax}};
    add_polygon_to_mask(mask, slab_vertices, Lx, Ly, Nx, Ny, dx, true);

    // Construct the air holes by setting "mask = false"
    float xc_hole_start = xc - a_lattice * (N_cavity - 1) / 2;
    float yc_hole_start = yc - a_lattice * (N_cavity - 1) / 2;
    for (int i = 0; i < N_cavity; ++i)
    {
        for (int j = 0; j < N_cavity; ++j)
        {
            float xc_hole = xc_hole_start + i * a_lattice;
            float yc_hole = yc_hole_start + j * a_lattice;
            add_circle_to_mask(mask, xc_hole, yc_hole, r_hole, Lx, Ly, Nx, Ny, dx, false);
        }
    }
}
...
// Function for setting C, D matrices used in FDTD
void set_FDTD_matrices_3D_structure(std::vector<float>& Cax, std::vector<float>& Cbx, std::vector<float>& Cay, std::vector<float>& Cby, std::vector<float>& Caz, std::vector<float>& Cbz, 
    std::vector<float>& Dax, std::vector<float>& Dbx, std::vector<float>& Day, std::vector<float>& Dby, std::vector<float>& Daz, std::vector<float>& Dbz, 
    int Nx, int Ny, int Nz, float dx, float dt, bool* mask,
    Complex eps_air, Complex eps_structure, 
    ...
    for (int i = 0; i < Nx; ++i) 
    {
        ...
        for (int j = 0; j < Ny; ++j) 
        {
            ...
            for (int k = 0; k < Nz; ++k)
            {
                ...
                // Now fill in all the C matrices (used for E field)
                // Notice that in this version we absorbed the "dx" into Cb&Db matrices
                float tmp_x = sigma_e_xx * dt / (2.0 * eps_xx * eps0);

                Cax[idx] = (1.0 - tmp_x) / (1.0 + tmp_x);
                Cbx[idx] = (dt / (eps_xx * eps0)) / (1.0 + tmp_x) / dx;

                float tmp_y = sigma_e_yy * dt / (2.0 * eps_yy * eps0);

                Cay[idx] = (1.0 - tmp_y) / (1.0 + tmp_y);
                Cby[idx] = (dt / (eps_yy * eps0)) / (1.0 + tmp_y) / dx;

                float tmp_z = sigma_e_zz * dt / (2.0 * eps_zz * eps0);

                Caz[idx] = (1.0 - tmp_z) / (1.0 + tmp_z);
                Cbz[idx] = (dt / (eps_zz * eps0)) / (1.0 + tmp_z) / dx;

                // Now fill in all the D matrices (used for H field)
                tmp_x = sigma_h_xx * dt / (2.0 * mu_xx * mu0);

                Dax[idx] = (1.0 - tmp_x) / (1.0 + tmp_x);
                Dbx[idx] = (dt / (mu_xx * mu0)) / (1.0 + tmp_x) / dx;

                tmp_y = sigma_h_yy * dt / (2.0 * mu_yy * mu0);

                Day[idx] = (1.0 - tmp_y) / (1.0 + tmp_y);
                Dby[idx] = (dt / (mu_yy * mu0)) / (1.0 + tmp_y) / dx;

                tmp_z = sigma_h_zz * dt / (2.0 * mu_zz * mu0);

                Daz[idx] = (1.0 - tmp_z) / (1.0 + tmp_z);
                Dbz[idx] = (dt / (mu_zz * mu0)) / (1.0 + tmp_z) / dx;
            }
        }
    }
}
