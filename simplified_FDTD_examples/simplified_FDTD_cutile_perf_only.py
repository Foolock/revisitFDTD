...
def update_e_inplace_kernel(
  ...
  interior = (idx < (Nx * Ny * Nz)) & (i > 0) & (i < Nx - 1) & (j > 0) & (j < Ny - 1) & (k > 0) & (k < Nz - 1)
...
class GDiamondCuTile:
  def __init__(self, Nx: int, Ny: int, Nz: int):
    if cp is None:
      raise RuntimeError("CuPy is required because cuTile kernels operate on CuPy device arrays.")
    if ct is None:
      raise RuntimeError("cuTile Python is required. Install cuda-tile.")
  ...
  def run_cutile(self, num_timesteps: int, tile_size: int = 256, warmup_steps: int = 3):
    ...
    def one_step(t: int):
      Mz[mz_idx] = cp.float32(self.M_source_amp * np.sin(self.SOURCE_OMEGA * np.float32(t) * self.dt))
      ct.launch(
        stream,
        grid,
        update_e_inplace_kernel,
        (
          Ex, Ey, Ez,
          Hx, Hy, Hz,
    ...
    for t in range(min(warmup_steps, num_timesteps)):
      one_step(t)
    stream.synchronize()
    ...
    print(f"cuTile runtime: {kernel_runtime}s")
    print(f"cuTile performance: {(self._Nx * self._Ny * self._Nz / 1.0e6 * num_timesteps) / kernel_runtime} Mcells/s")
  ...
  def compare_with_numpy_vectorized(self, ref_module_path: str, num_timesteps: int):
    ...
    Mz = ref_sim._Mz.reshape(Nz, Ny, Nx).copy()
    ...
    start = time.perf_counter()
    for t in range(num_timesteps):
      Mz[mz_k, mz_j, mz_i] = np.float32(ref_sim.M_source_amp * np.sin(ref_sim.SOURCE_OMEGA * np.float32(t) * ref_sim.dt))
    ...
    print(f"NumPy vectorized performance: {(self._Nx * self._Ny * self._Nz / 1.0e6 * num_timesteps) / numpy_runtime} Mcells/s")
...
def parse_args():
  parser = argparse.ArgumentParser(description="cuTile-only performance benchmark for the simplified 3D FDTD example.")
  parser.add_argument("Nx", type=int)
  parser.add_argument("Ny", type=int)
  parser.add_argument("Nz", type=int)
  parser.add_argument("num_timesteps", type=int)
  parser.add_argument("--tile-size", type=int, default=256, help="1D cuTile tile size. Must be a power of two.")
  parser.add_argument("--warmup-steps", type=int, default=3, help="Warmup steps before timing cuTile.")
...
def main():
  ...
  sim.run_cutile(args.num_timesteps, tile_size=args.tile_size, warmup_steps=args.warmup_steps)
