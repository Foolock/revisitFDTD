...
class GDiamondCuTileCUDAStyle:
  ...
  def run_cutile_cuda_style(self, num_timesteps: int, tile_size: int = 256, warmup: bool = True):
    ...
    if warmup:
      ...
      w_Ex = cp.zeros(N, dtype=cp.float32)
      w_Ey = cp.zeros(N, dtype=cp.float32)
      w_Ez = cp.zeros(N, dtype=cp.float32)
      w_Hx = cp.zeros(N, dtype=cp.float32)
      w_Hy = cp.zeros(N, dtype=cp.float32)
      w_Hz = cp.zeros(N, dtype=cp.float32)

      w_Jx = cp.zeros(N, dtype=cp.float32)
      w_Jy = cp.zeros(N, dtype=cp.float32)
      w_Jz = cp.zeros(N, dtype=cp.float32)
      w_Mx = cp.zeros(N, dtype=cp.float32)
      w_My = cp.zeros(N, dtype=cp.float32)
      w_Mz = cp.zeros(N, dtype=cp.float32)

      w_Cax = cp.asarray(self._Cax)
      w_Cay = cp.asarray(self._Cay)
      w_Caz = cp.asarray(self._Caz)
      w_Cbx = cp.asarray(self._Cbx)
      w_Cby = cp.asarray(self._Cby)
      w_Cbz = cp.asarray(self._Cbz)

      w_Dax = cp.asarray(self._Dax)
      w_Day = cp.asarray(self._Day)
      w_Daz = cp.asarray(self._Daz)
      w_Dbx = cp.asarray(self._Dbx)
      w_Dby = cp.asarray(self._Dby)
      w_Dbz = cp.asarray(self._Dbz)

      mz_idx = int(self._source_idx)
      w_Mz[mz_idx] = cp.float32(self.M_source_amp * np.sin(self.SOURCE_OMEGA * np.float32(0) * self.dt))

      ct.launch(
        stream, grid,
        update_e_kernel_inplace,
        (
          w_Ex, w_Ey, w_Ez, w_Hx, w_Hy, w_Hz,
          w_Cax, w_Cbx, w_Cay, w_Cby, w_Caz, w_Cbz,
          w_Jx, w_Jy, w_Jz,
          float(self._dx),
          Nx, Ny, Nz, tile_size,
        ),
      ...
      ct.launch(
        ...
        (
          w_Ex, w_Ey, w_Ez,
          w_Hx, w_Hy, w_Hz,
          w_Dax, w_Dbx, w_Day, w_Dby, w_Daz, w_Dbz,
          w_Mx, w_My, w_Mz,
          float(self._dx),
          Nx, Ny, Nz,
          tile_size,
      ...
      del w_Ex, w_Ey, w_Ez, w_Hx, w_Hy, w_Hz
      del w_Jx, w_Jy, w_Jz, w_Mx, w_My, w_Mz
      del w_Cax, w_Cay, w_Caz, w_Cbx, w_Cby, w_Cbz
      del w_Dax, w_Day, w_Daz, w_Dbx, w_Dby, w_Dbz
    ...
    print(f"warmup runtime: {warmup_runtime}s")
    print(f"end to end runtime: {total_runtime}s")
    print(
      f"end to end throughput: {(self._Nx * self._Ny * self._Nz / 1.0e6 * num_timesteps) / total_runtime} Mcells/s"
    )
    print(
      f"kernel throughput: {(self._Nx * self._Ny * self._Nz / 1.0e6 * num_timesteps) / kernel_runtime} Mcells/s"
    )

    total_s = total_runtime
    h2d_s = h2d_runtime
    kernel_s = kernel_runtime
    d2h_s = d2h_runtime
    print("runtime breakdown (%):")
    print(f"  H2D memcpy : {100.0 * h2d_s / total_s:.2f}%")
