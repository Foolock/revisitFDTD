...
class GDiamondNumpy:
  ...
  def _finalize_from_host_arrays(self, Ex_temp, Ey_temp, Ez_temp, Hx_temp, Hy_temp, Hz_temp, seq_runtime, num_timesteps):
    ...
    print(f"number of pixels: {self._Nx * self._Ny * self._Nz}")
    print(f"numpy runtime: {seq_runtime}s")
    print(
      "numpy throughput: "
  ...
  def update_FDTD_seq_figures_cuda(self, num_timesteps: int, outdir: str):
    ...
    # include all pending GPU work before stopping timer
    cp.cuda.Stream.null.synchronize()

    Ex_host = cp.asnumpy(Ex_temp)
    Ey_host = cp.asnumpy(Ey_temp)
    Ez_host = cp.asnumpy(Ez_temp)
    Hx_host = cp.asnumpy(Hx_temp)
    Hy_host = cp.asnumpy(Hy_temp)
    Hz_host = cp.asnumpy(Hz_temp)
...
def parse_args():
  if len(sys.argv) != 6:
    print(f"Usage: {sys.argv[0]} Nx Ny Nz num_timesteps mode")
  ...
  return Nx, Ny, Nz, num_timesteps, mode


def main():
  Nx, Ny, Nz, num_timesteps, mode = parse_args()

  fig_dir = "numpy_figures"
