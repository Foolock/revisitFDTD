...
class GDiamondCuTile:
  ...
  def run_cutile(self, num_timesteps: int, outdir: str, tile_size: int = 256):
    ...
    for t in range(num_timesteps):
      ...
      if t % record_stride == 0:
        ...
        k_mid = Nz // 2
        hz_mid = cp.asnumpy(Hz_a[k_mid * Nx * Ny:(k_mid + 1) * Nx * Ny])
        field_filename = os.path.join(outdir, f"Hz_cutile_{t:04d}.png")
        self.save_field_png(
          hz_mid,
          field_filename,
          Nx,
  ...
  def compare_with_numpy_reference(self, ref_module_path: str, num_timesteps: int, mode: str = "vectorized"):
    ...
    tmp_dir = "_tmp_numpy_ref_figs"
    if mode == "indexing":
      ref_sim.update_FDTD_seq_figures_indexing(num_timesteps, tmp_dir)
    elif mode == "vectorized":
      ref_sim.update_FDTD_seq_figures_vectorized(num_timesteps, tmp_dir)
    elif mode == "cuda":
      ref_sim.update_FDTD_seq_figures_cuda(num_timesteps, tmp_dir)
    else:
      raise ValueError("reference mode must be one of: indexing, vectorized, cuda")
    ...
    print("\nComparing cuTile result against NumPy reference")
...
def parse_args():
  parser = argparse.ArgumentParser(description="cuTile version of the simplified 3D FDTD example.")
  ...
  parser.add_argument("--cpp-dir", type=str, default="seq_fields_cpp", help="Directory containing Ex_seq.bin, ..., Hz_seq.bin.")
  parser.add_argument("--numpy-ref", type=str, default=None, help="Path to the NumPy reference script.")
  parser.add_argument("--numpy-mode", type=str, default="vectorized", choices=("indexing", "vectorized", "cuda"))
  parser.add_argument("--skip-cpp-compare", action="store_true")
  parser.add_argument("--skip-numpy-compare", action="store_true")
...
def main():
  ...
  sim.run_cutile(args.num_timesteps, outdir=args.fig_dir, tile_size=args.tile_size)

  if not args.skip_cpp_compare:
    print(f"\nComparing against C++ binary in {args.cpp_dir}/")
    sim.compare_with_cpp_bin(args.cpp_dir)

  if (not args.skip_numpy_compare) and args.numpy_ref is not None:
    sim.compare_with_numpy_reference(args.numpy_ref, args.num_timesteps, mode=args.numpy_mode)
